function pilotPara = SinglePilotParameters(M,N,L,pilotPow)
% FUNCTION: determine the pilot parameters for single pilot design for OTFS

% single pilot parameters position
pilotPara = struct();
pilotPara.Position = ceil([M/2,N/2]);
pilotPara.GuardRange = [pilotPara.Position(1)-2*L, ...
        pilotPara.Position(1)+2*L, ...
        1,...
        N];
    pilotPara.RxPilotRange = [pilotPara.Position(1)-L, ...
        pilotPara.Position(1)+L, ...
        1,...
        N];
pilotPara.numGuardBins = (pilotPara.GuardRange(2) - pilotPara.GuardRange(1) + 1)*...
    (pilotPara.GuardRange(4) - pilotPara.GuardRange(3) + 1);
pilotPara.pilotPow = pilotPow; % pilot relative power(dB)
...
pilotIndex = sub2ind([M,N],pilotPara.Position(1),pilotPara.Position(2));
